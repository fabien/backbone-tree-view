...
BackTree.Item = (function(superClass) {
  ...
  Item.prototype.hasCheckbox = function() {
    var checkbox, checkboxAttr, hasCheckbox;
    checkbox = this.settings.get('checkbox');
    checkboxAttr = !this.model.has('checkbox') || this.model.get('checkbox');
    hasCheckbox = checkbox && checkboxAttr;
    if (hasCheckbox && checkbox === 'leaf') {
      hasCheckbox = !this.model.hasChildren();
    } else if (hasCheckbox && checkbox === 'branch') {
      hasCheckbox = this.model.hasChildren();
    }
    return hasCheckbox;
  };
